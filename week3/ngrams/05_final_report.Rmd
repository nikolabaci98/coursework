...
```{r load-counts}
year_counts <- read_tsv(here("year_counts.tsv"), col_names = c("term", "year", "volume", "book_count_term"))
total_counts <- read_csv(here("total_counts.csv"), col_names = c("year", "total_volume", "page_count", "book_count_total"))
...
term_by_total_count <-  year_counts %>%
  left_join(total_counts, by="year")

# the proportion is the number of times a term appeared divided by all
# the other terms that appeared on the same year
term_by_total_count <-  term_by_total_count %>%
  mutate(proportion_term_in_year = volume / total_volume) 
...
term_by_total_count %>%
  ...
  ggplot() +
  geom_line(aes(x = year, y = proportion_term_in_year, color = factor(term)), size = 1)+
...
```{r compute-peaks}
peak_year_per_term <- term_by_total_count %>% 
  filter(year >= term) %>%
  group_by(term) %>%
  filter(proportion_term_in_year == max(proportion_term_in_year)) %>% 
  select(c("term", "year", "proportion_term_in_year"))

future_years_only <-  term_by_total_count %>%
   filter(year >= term)

peak_and_future_years <- future_years_only %>%
  left_join(peak_year_per_term, by="term", suffix = c("", "_peak")) %>%
...
half_life_year <- peak_and_future_years %>%
  group_by(term) %>%
  filter(proportion_term_in_year < proportion_term_in_year_peak / 2) %>%
  summarize(half_life = min(year)) 

half_life_period <- half_life_year %>%
  left_join(peak_year_per_term, by="term") %>%
  mutate(period = half_life - year)
```
...
half_life_period %>%
  ggplot()+
  geom_jitter(aes(x = term, y = period))+
  geom_smooth(aes(x = term, y = period))
